...
from qg.QGTrainer import QGTrainer
...
def main():
    ...
    if data_args.eval_beams == 1 and data_args.do_sample == False:
        ...
        if data_args.top_p != 1.0:
            output_dir += f'-top_p{data_args.top_p}-temperature{training_args.temperature}'
    ...
    # dlc save
    # model.save_pretrained(f'/mnt/ruzhen/ruzhen/model/{model_args.model_name_or_path}')
    # tokenizer.save_pretrained(f'/mnt/ruzhen/ruzhen/model/{model_args.model_name_or_path}')
    # config.save_pretrained(f'/mnt/ruzhen/ruzhen/model/{model_args.model_name_or_path}')
    ...
    if data_args.task == 'paraphrase_generation':
        from datasets_tasks import Paraphrase_Generation_Dataset as My_Dataset
    elif data_args.task == 'QG' and 'newsqa' in data_args.train_data_dir:
        from datasets_tasks import Newsqa_QG_Dataset as My_Dataset
    elif data_args.task == 'QG':
        from datasets_tasks import SQuAD_QG_Dataset as My_Dataset
    # Get datasets
    train_dataset = (My_Dataset(
            ...
            is_use_skeleton=data_args.is_use_skeleton,
